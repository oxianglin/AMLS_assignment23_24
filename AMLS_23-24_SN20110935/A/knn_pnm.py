...
def knn_pnm():
    ...
    dir.append('PneumoniaMNIST')
    dir.append('pneumoniamnist.npz')
    ...
    for item in lst:    
        if (item == 'train_images'):
            ...
            X_train = X_train_images.reshape(X_train_images.shape[0], X_train_images.shape[1]*X_train_images.shape[2])
        if (item == 'train_labels'):
            y_train = npz_file[item].flatten()
        if (item == 'val_images'):
            X_val_images = npz_file[item]
            X_val = X_val_images.reshape(X_val_images.shape[0], X_val_images.shape[1]*X_val_images.shape[2])
        if (item == 'val_labels'):
