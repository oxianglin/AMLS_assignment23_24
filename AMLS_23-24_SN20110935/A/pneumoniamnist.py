...
class data_set(data.Dataset):
    ...
    def __init__(self, split, transform=None, target_transform=None, as_rgb=False): #Initialise dataset.
        ...
        if (DEBUG and split == 'train'):
            lst = npz_file.files
            dbg_print('pneumoniamnist.npz contans:')
            for item in lst:
                dbg_print(item)
            for item in lst:    
    ...
    def search_data(self):
        dir = os.path.dirname(__file__) #<Directory of this file> which is <upper directories/amls/A>.
        ds = dir.split(os.sep) #Split string at os-dependent directory seperator (e.g. \\)
...
class Pneumoniamnist():
    def __init__(self, debug):
        ...
        print('Dataset objects to be initiated from data_set class contain pneumoniamnist data')
